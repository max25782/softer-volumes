...
export async function POST(req: Request) {
  ...
  if (body.event_type === 'PAYMENT.CAPTURE.COMPLETED' && body.resource?.status === 'COMPLETED') {
    const purchaseMetadata = parsePayPalPurchaseMetadata(body.resource.custom_id)
    ...
    if (purchaseMetadata && body.resource.id && amount > 0) {
      ...
      if (purchase === null) {
        console.error('PayPal webhook did not match a published guide price', body.resource.id)
      }
    }
  }
