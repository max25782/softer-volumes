...
export async function findGuideByIdOrSlug(input: {
  ...
  const publishedWhere = input.publishedOnly === true ? { isPublished: true } : {}
  const include = { _count: { select: { places: true } } }

  if (input.guideId !== undefined) {
    const guide = await prisma.guide.findFirst({
      where: { id: input.guideId, ...publishedWhere },
      include,
    })
  ...
  return prisma.guide.findFirst({
    where: { slug: input.guideSlug, ...publishedWhere },
    include,
  })
}
