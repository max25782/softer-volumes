...
import { getCompletedPayPalCaptureDetails, type PayPalCapture } from './paypal.ts'
...
test('uses capture-level metadata when PayPal returns it there', () => {
  assert.deepEqual(
...
test('rejects PayPal captures with malformed metadata', () => {
  assert.equal(
    getCompletedPayPalCaptureDetails(
      createCapture({
        purchase_units: [
          {
            custom_id: 'user_1:guide_1:extra',
            payments: {
              captures: [
                {
                  id: 'capture_123',
